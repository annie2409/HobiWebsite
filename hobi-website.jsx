      ...
      <Section>
        ...
        <div className="two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "60px", alignItems: "center", marginBottom: "80px" }}>
          <div className="reveal">
            ...
            <p style={{ color: C.gray600, fontSize: "16px", lineHeight: 1.8, marginBottom: "28px" }}>
              WanderSync turns the chaos of group travel planning into a seamless, shared experience. Create trips, invite members, suggest destinations, vote on options, and build your day-by-day itinerary together in real time.
            </p>
            <div style={{ display: "flex", gap: "20px", flexWrap: "wrap", marginBottom: "28px" }}>
              {[["10k+", "Sync Updates"], ["Google", "Maps API"], ["Real-time", "Socket.io"]].map(([val, lab]) => (
          ...
          <div className="reveal" style={{ display: "flex", justifyContent: "center" }}>
            <div style={{ width: "280px", background: C.gray900, borderRadius: "36px", padding: "20px", boxShadow: "0 32px 80px rgba(0,0,0,0.2)" }}>
              ...
              {[
                { time: "09:00 AM", name: "Senso-ji Temple", emoji: "🏮", detail: "1.5h dwell · Quiet morning walk", trans: "🚶 Walking route directions" },
                { time: "11:30 AM", name: "Ichiran Ramen", emoji: "🍜", detail: "1.0h dwell · Lunch suggestion", trans: "🚇 Transit directions" },
                { time: "01:00 PM", name: "Ueno Park", emoji: "🌸", detail: "2.0h dwell · Voted attraction ✓", trans: null },
              ].map((item, idx) => (
                <div key={idx}>
        ...
        {/* Features */}
        <SectionHeader label="Features" title="Designed for Real-Time Sync" subtitle="Powerful tools that turn scattered recommendations into a structured, collaborative journey." />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "24px" }}>
          {[
            { icon: "🤖", title: "Real-Time Sync", desc: "Uses Socket.io to keep itinerary changes, sorting orders, and planning details synced across all devices instantly." },
            { icon: "👥", title: "Presence Indicators", desc: "See who's currently online planning with you, typing in the group chat, or editing specific stops on the itinerary." },
            { icon: "🗳️", title: "Collaborative Voting", desc: "Suggest spots and let everyone vote on their favorite activities so you reach a group consensus effortlessly." },
            { icon: "🗺️", title: "Interactive Google Maps", desc: "See your plan pinned automatically on the map with directions and route details rendered between each stop." },
            { icon: "💬", title: "Group Chat Workspace", desc: "An integrated chat panel allows you to bounce ideas around, debate routes, and make decisions without switching apps." },
            { icon: "↕️", title: "Drag-and-Drop planning", desc: "A highly responsive day-by-day scheduling board makes organizing and reordering items quick and fluid." },
          ].map((f) => (
            <div className="reveal" key={f.title}>
              <Card>
                <div style={{ fontSize: "32px", marginBottom: "14px" }}>{f.icon}</div>
        ...
        <div className="reveal" style={{ marginTop: "60px", textAlign: "center", padding: "60px", background: `linear-gradient(135deg, ${C.blueLight}, ${C.greenLight})`, borderRadius: "24px" }}>
          ...
          <Btn onClick={() => nav("Contact")} style={{ background: `linear-gradient(135deg, ${C.blue}, ${C.green})` }}>Get Started →</Btn>
        </div>
      </Section>
    </div>
  );
